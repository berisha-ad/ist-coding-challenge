...
const SOAP_ACTION = "http://www.uid.admin.ch/xmlns/uid-wse/IPublicServices/ValidateVatNumber";
export class VatServiceCH extends VatService {
    async check(_countryCode, vat) {
  ...
  <soap:Body>
    <uid:ValidateVatNumber>
      <uid:vatNumber>${vat}</uid:vatNumber>
        ...
        const m = xml.match(/<[^:]*ValidateVatNumberResult[^>]*>(true|false)<\/[^:]*ValidateVatNumberResult>/i);
        if (!m)
            throw new Error("CH service call failed");
        console.log(m);
